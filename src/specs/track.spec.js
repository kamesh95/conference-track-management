...
describe('Track', () => {
    ...
    describe('#prepareTrack()', () => {
        ...
        context('With afternoon duration as 0', () => {
            before(() => _before(0));

            it('Should add lunch in morning slot and networking with networking deadline in afternoon slot', () => {
                expect(track.morningSlot[0]).includes('Lunch');
                expect(track.afternoonSlot[0]).includes(track.networkingDeadline.toString());
                expect(result.length).to.be.equal(2);
            });
        });

        context('With afternoon duration less than the min and max afternoon duration', () => {
            before(() => _before(10));

            it('Should add lunch in morning slot and networking with afternoon time in afternoon slot', () => {
                expect(track.morningSlot[0]).includes('Lunch');
                expect(track.afternoonSlot[0]).includes(track.afternoonTime.toString());
                expect(result.length).to.be.equal(2);
            });
        });
